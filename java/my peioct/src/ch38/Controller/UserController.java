	...
	public Map<String, Object> execute(Map<String, Object> params) {
		...
		try {
			...
			case 1:
				...
				System.out.println("[No-1 회원가입] : 유호성 검증 확인 :" + isOk);
				if (!isOk) {
					response.put("status", false);
//					response.put("message", "유효성 체크 오류발생");
	...
	private boolean isValid(UserDto userDto) {
		...
		if (userDto.getUserid().matches("^[0-9].*")) {
			System.out.println("[INVALID] userid의 첫 문자로 숫자가 들어올 수 없습니다.");
			response.put("error", " userid의 첫 문자로 숫자가 들어올 수 없습니다.");
			return false;
		}
		// NULL 체크 / 데이터(자료) 수준에서의 의미있는 데이터가 포함되어져 있는지 여부
		// userid는 첫 문자가 숫자인지 여부 - / or 길이가 1글자인지 등등..
		// username은 첫 문자가 숫자인지 여부 -
